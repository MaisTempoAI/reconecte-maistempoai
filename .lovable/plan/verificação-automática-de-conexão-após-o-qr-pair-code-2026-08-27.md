...
1. Logo que o QR (ou o código de pareamento) aparece, o app começa a perguntar ao servidor (a cada 10 segundos) se o número já conectou.
...
- Em `src/routes/index.tsx`: `setInterval` de 10s iniciado quando `result.kind` é `qr` ou `paircode`, limpo ao conectar, ao gerar novo código, ao voltar/reset e no unmount. Nada de polling nos estados de erro/conectado.
